...
def run_simulation(n_nodes_r, n_nodes_z):
    ...
    with open(f"results_{n_r}x{n_z}.pkl", "wb") as f:
        pkl.dump(large_tank.data, f)
    print(f'Results saved to results_{n_r}x{n_z}.pkl')

# Run the simulation for different grid sizes
r_grid = np.array([32, 64])
z_grid = np.array([32, 64])
simulation_times = []
desviation_times = []

for n_r, n_z in zip(r_grid, z_grid):
    print(f"Running simulation for grid size {n_r}x{n_z}...")
    run_simulation(n_r, n_z)
